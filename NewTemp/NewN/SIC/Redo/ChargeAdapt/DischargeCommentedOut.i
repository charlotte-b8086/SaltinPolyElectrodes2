...
cm0 = '${fparse 0.14645/rho_M}'
cmcoef = '${fparse -0.5*rho_M}'

[Mesh]
#[restart]
  type = FileMesh#Generator
  file = 'Charge_out.e'#out_cp/0035-mesh.cpa.gz#'Charge_out.e'
  #use_for_exodus_restart = true
#[]
[]
#
#[Problem]
#  # Omit the file extension and use the specific step or LATEST
#  restart_file_base = out_cp/LATEST
#[]
[Problem]
  type = FEProblem
  restart_file_base = 'Charge_out_cp/0035'
[]

[UserObjects]
  [solution_reader]
    type = SolutionUserObject
    mesh = out_cp/0035-mesh.cpa.gz
    time_index = LATEST
  []
[]
...
[AuxKernels]
  ...
  [./cm_ODE_aux]
    ...
    variable = cm_ODE
    coupled_variables = 'cp cn cm grad_cp grad_cn grad_cm grad_psi'
    expression = '(1/${N}+${rho_M}*cm/sqrt(1e-1+(1-cp-cn-cm)^2))*grad_cm +${rho_M}* (cm/(sqrt(1e-1+(1-cp-cn-cm)^2)))*(grad_cp + grad_cn) - cm * grad_psi'
  [../]
  [./rho_ODE_aux]
    type = ParsedAux
    variable = rho_ODE
...
#    type = SolutionIC
#    variable = cp
#    solution_object = solution_reader
#    from_variable = cp
#  []
#  [cn_init]
#    type = SolutionIC
...
[Materials]  
  ...
  [cation_electrodiffusivity]
    ...
    block = '1'
  []
  [poly_electrodiffusivity]
    type = ADParsedMaterial
    coupled_variables = 'cm'
    expression = '-${D_m}*cm'
    property_name = poly_electrodiff
    block = '1'
  []
...
[VectorPostprocessors]
  active = 'cp_sample cn_sample cm_sample rho_sample ci_sample psi_sample psi_e_sample'
  [cp_sample]
