...
def compute_params(t0, N):
  ...
  # So initial cation concentration matches DIC case
  phiP    = phiSalt/(1+rhoP/rhoN)
  phiN = (rhoP/rhoN)*phiP
  phiPoly = 1-phiP-phiN-phiVM
  phiPoly = phiPoly + phiN

  # For local electroneutrality 
  rhoPoly = rhoP * phiP/phiPoly
  print(phiP, phiPoly, (rhoP* phiP /phiPoly)-rhoPoly, phiP-(rhoPoly/rhoP)*phiPoly) 
