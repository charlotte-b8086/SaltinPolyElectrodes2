import pandas as pd
import numpy as np
#../../../../../../raspberry_app-opt -i Charge.i eps=1.16e-5 eps_e=1.16e-5 delta=2 D_p=1 D_n=0 D_m=0 rho_M=0.000118 rho_I=1 I_ex=0.234 D_i=4.81 I=-0.0117 D_e=481 nx_electrode=4000 nx_electrolyte=4000 N=1000 -w
...
def main():
  ...
  for idx in range(2, len(timeSteps)):
    ...
    vm0 = softfloor(1-0.14645-cp0)
    vm1 = softfloor(1-0.14645-cp1)
    vl0 = softfloor(1-ci0)
    vr1 = softfloor(1-ci1)
  ...
  for idx in range(1, len(timeSteps)):
    ...
    dfcm  = pd.read_csv(f'Charge_out_cm_sample_{timeSteps[idx]}.csv')
    dfdpsidx = pd.read_csv(f'Charge_out_dpsidx_sample_{timeSteps[idx]}.csv')

    RHSpsi = 0.5*(dfcp['cp']-rhoM*dfcm['cm'])
