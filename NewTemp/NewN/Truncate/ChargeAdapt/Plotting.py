...
### Troubleshooting
    df  = pd.read_csv(f"{args.prefixD}.csv")
    ...
    dfcp  = pd.read_csv(f"Charge_out_cp_sample_0015.csv")
    dfcn  = pd.read_csv(f"Charge_out_cn_sample_0015.csv")
    dfcm  = pd.read_csv(f"Charge_out_cm_sample_0015.csv")
    ...
    dfci  = pd.read_csv(f"Charge_out_ci_sample_0015.csv")
    dfce  = pd.read_csv(f"Charge_out_rho_sample_0015.csv")
    ciMskL = dfci['x']<=0
    ciMskR = dfci['x']>=1
    dfciL = dfci[ciMskL]
    dfciR = dfci[ciMskR]
    ...
    dfpsiM = pd.read_csv(f"Charge_out_psi_sample_0015.csv")
    dfpsiE = pd.read_csv(f"Charge_out_psi_e_sample_0015.csv")
    psiLMsk = dfpsiE['x']<=0
    psiRMsk = dfpsiE['x']>=1
    ...
    axes[0, 0].plot(t_v, v, color="tab:blue")
    ...
    axes[0, 1].plot(t_iR, iR, color="tab:orange")
    ...
    axes[0, 2].plot(iR, v, color="tab:green", marker="o", markersize=2)
    axes[0, 2].set_xlabel(
    r'$\frac{\int_1^{1+\delta}\phi_{i_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    r' \, \mathrm{d}\hat{x}}{\int_1^{1+\delta}\phi_{i_\mathrm{R}}'
    r'\left(\hat{x},\hat{t}\right)+\phi_{v_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    ...
    axes[1, 0].plot(t_vD, vD, color="tab:blue")
    ...
    axes[1, 1].plot(t_iRD, iRD, color="tab:orange")
    ...
    axes[1, 2].plot(iRD, vD, color="tab:green", marker="o", markersize=2)
    axes[1, 2].set_xlabel(
    r'$\frac{\int_1^{1+\delta}\phi_{i_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    r' \, \mathrm{d}\hat{x}}{\int_1^{1+\delta}\phi_{i_\mathrm{R}}'
    r'\left(\hat{x},\hat{t}\right)+\phi_{v_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    ...
    axesV.plot(tau*t_v, kB*temp*v/eC, color='k', marker='o', markersize=2, label="Charge")
    axesV.plot(tau*t_vD, kB*temp*vD/eC, color="tab:blue", marker='o', markersize=2, label="Discharge")
    axesV.set_xlabel(r"${t}$ (Seconds)")
    axesV.set_ylabel(
    r'$\frac{k_BT}{e} \left[ \ln\left(\frac{\rho_{e_\mathrm{L}}}{\rho_+^o}\right) -'
    r' \frac{e\psi_\mathrm{L}}{k_BT}|_{-\delta}-'
    r' \left[\ln\left(\frac{\rho_{e_\mathrm{R}}}{\rho_+^o}\right)'
    r' -\frac{e\psi_\mathrm{R}}{k_BT}\right]|_{1+\delta}\right]$ (Volts)'
)
    axesV.set_title("Voltage vs Time")
    axesV.legend()
    figV.tight_layout()
    figV.savefig("V(t)Dimensionalized", dpi=150)

    figU, axesU = plt.subplots(figsize=(16, 4.5))
    axesU.plot(tau*t_v, iR, color='k', marker='o', markersize=2, label="Charge")
    axesU.plot(tau*t_vD, iRD, color="tab:blue", marker='o', markersize=2, label="Discharge")
    axesU.set_xlabel(r"${t}$ (Seconds)")
    axesU.set_ylabel(
    r'$\frac{\int_1^{1+\delta}\phi_{i_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    r' \, \mathrm{d}\hat{x}}{\int_1^{1+\delta}\phi_{i_\mathrm{R}}'
    r'\left(\hat{x},\hat{t}\right)+\phi_{v_\mathrm{R}}\left(\hat{x},\hat{t}\right)'
    r'\,\mathrm{d}\hat{x}}$ (Unitless)'
)
    axesU.set_title("Utilization vs Time")
    axesU.legend()
    figU.tight_layout()
    figU.savefig("U(t)Dimensionalized", dpi=150)
